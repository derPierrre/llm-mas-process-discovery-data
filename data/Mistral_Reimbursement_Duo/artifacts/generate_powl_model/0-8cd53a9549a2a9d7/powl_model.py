...
expense_approval_choice = gen.xor(auto_approval, manual_review)
manual_review_choice = gen.xor(approve_expense, reject_expense)

# Define partial orders
initial_process = gen.partial_order(dependencies=[(expense_submission, confirmation_notification), (confirmation_notification, first_submission_check)])

expense_review_process = gen.partial_order(dependencies=[(manual_review, manual_review_choice)])

approval_process = gen.partial_order(dependencies=[(approve_expense, initiate_deposit), (initiate_deposit, send_approval_notice)])

full_process = gen.partial_order(dependencies=[(initial_process, expense_approval_choice), (expense_approval_choice, expense_review_process), (expense_review_process, approval_process), (approval_process, request_rectification)])
