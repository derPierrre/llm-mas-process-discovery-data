...
# The 'do' part is the system review. The 'redo' part is the notification and correction by employee.
...
# The loop itself represents the review and potential corrections. If it completes successfully (no more redo), then approval follows.
...
# The 'do' part is the MA review. The 'redo' part is the FT notification and employee correction.
...
# Approved outcome for Path B (payment processing by FT)
...
# Decision by MA after the review/correction loop cycle has completed (i.e., MA is not sending for another correction round).
# This XOR represents the MA's final decision point: either the report is now good for approval, or it's to be definitively rejected.
...
# XOR based on amount, choosing between Full Path A and Full Path B
