...
# This includes Notification for Replacement and then Delivery of Replacement.
successful_replacement_sequence = gen.partial_order(dependencies=[
    (notify_for_replacement_activity, deliver_replacement_activity)
])

# Model the conditional execution of the successful replacement sequence.
# After 'Monitor & Inspect Return', if inspection is successful, the 'successful_replacement_sequence' occurs.
# Otherwise (e.g., inspection fails), this sequence is skipped (None).
conditional_successful_outcome = gen.xor(successful_replacement_sequence, None)
...
one_exchange_cycle = gen.partial_order(dependencies=[
    ...
    (monitor_inspect_return, conditional_successful_outcome) # The conditional outcome follows inspection
])
